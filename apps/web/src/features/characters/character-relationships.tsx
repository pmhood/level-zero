'use client';

import {
  isLineageRelation,
  type Entity,
  type NeighborEdge,
  type RelationType,
} from '@level-zero/domain';
import { Button, EmptyState, Field, LinkIcon, Select, Tag } from '@level-zero/ui';
import { useState } from 'react';

import { entityTypeLabel, relationLabel } from '@/features/entities/entity-presentation';
import { useReferenceableEntities } from '@/features/entities/use-entities';
import { apiErrorMessage } from '@/lib/api';

import { useCharacterLinks, useLinkCharacter, useUnlinkCharacter } from './use-characters';

/**
 * The relations a designer draws by hand from a character.
 *
 * Lineage relations are left out on purpose: they record how something came to
 * exist and are written by promotion and generation, not chosen from a menu.
 */
const LINKABLE_RELATIONS = [
  'belongs_to',
  'appears_in',
  'references',
  'depends_on',
  'contains',
  'implements',
] as const satisfies readonly RelationType[];

/** Visuals are edges too, and have their own tab; this one is everything else. */
function isRelationshipEdge(edge: NeighborEdge): boolean {
  return edge.entity.type !== 'asset_reference';
}

function EdgeRow({
  edge,
  onUnlink,
  unlinking,
}: {
  edge: NeighborEdge;
  onUnlink: (relationshipId: string) => void;
  unlinking: boolean;
}) {
  const label =
    edge.direction === 'outgoing'
      ? relationLabel(edge.relationship.relation)
      : `${relationLabel(edge.relationship.relation)} by`;

  // Lineage is history: the API refuses to unlink it, so nothing offers to.
  const removable = edge.direction === 'outgoing' && !isLineageRelation(edge.relationship.relation);

  return (
    <li className="flex items-center justify-between gap-2 rounded-md border border-border-subtle bg-raised px-3 py-2">
      <div className="min-w-0">
        <p className="truncate text-sm text-foreground">{edge.entity.name}</p>
        <p className="text-xs text-faint-foreground">
          {label} · {entityTypeLabel(edge.entity.type)}
        </p>
      </div>
      <div className="flex shrink-0 items-center gap-1.5">
        {edge.entity.status === 'archived' && <Tag>Archived</Tag>}
        {removable && (
          <Button
            variant="ghost"
            size="sm"
            disabled={unlinking}
            onClick={() => onUnlink(edge.relationship.id)}
          >
            Unlink
          </Button>
        )}
      </div>
    </li>
  );
}

function LinkForm({ projectId, character }: { projectId: string; character: Entity }) {
  const [relation, setRelation] = useState<RelationType>('belongs_to');
  const [targetId, setTargetId] = useState('');
  const entitiesQuery = useReferenceableEntities(projectId);
  const linkCharacter = useLinkCharacter(projectId);

  const targets = (entitiesQuery.data?.items ?? []).filter(
    (candidate) => candidate.id !== character.id && candidate.status !== 'archived',
  );

  return (
    <form
      className="flex flex-col gap-2 border-t border-border-subtle pt-4"
      onSubmit={(event) => {
        event.preventDefault();
        if (!targetId) return;
        linkCharacter.mutate(
          { entityId: character.id, input: { targetEntityId: targetId, relation } },
          { onSuccess: () => setTargetId('') },
        );
      }}
    >
      <Field
        label="Link this character"
        hint="Factions, locations and mechanics stay where they are — this only draws the edge."
      >
        <Select
          aria-label="Relation"
          value={relation}
          onChange={(event) => setRelation(event.target.value as RelationType)}
        >
          {LINKABLE_RELATIONS.map((option) => (
            <option key={option} value={option}>
              {relationLabel(option)}
            </option>
          ))}
        </Select>
      </Field>

      <Select
        aria-label="Entity to link"
        value={targetId}
        onChange={(event) => setTargetId(event.target.value)}
      >
        <option value="">Choose an entity…</option>
        {targets.map((candidate) => (
          <option key={candidate.id} value={candidate.id}>
            {candidate.name} · {entityTypeLabel(candidate.type)}
          </option>
        ))}
      </Select>

      {linkCharacter.isError && (
        <p className="text-xs text-error">
          {apiErrorMessage(linkCharacter.error, 'Could not create that link.')}
        </p>
      )}

      <Button
        type="submit"
        size="sm"
        variant="secondary"
        disabled={!targetId || linkCharacter.isPending}
      >
        <LinkIcon className="size-4" />
        {linkCharacter.isPending ? 'Linking…' : 'Add link'}
      </Button>
    </form>
  );
}

/** Who and what this character is bound to, and the way to draw another edge. */
export function CharacterRelationships({
  projectId,
  character,
}: {
  projectId: string;
  character: Entity;
}) {
  const links = useCharacterLinks(projectId, character.id);
  const unlink = useUnlinkCharacter(projectId);

  if (links.isPending) {
    return <p className="text-sm text-muted-foreground">Loading relationships…</p>;
  }

  if (links.isError) {
    return (
      <div className="flex flex-col gap-2">
        <p className="text-sm text-error">{apiErrorMessage(links.error)}</p>
        <Button variant="secondary" size="sm" onClick={() => void links.refetch()}>
          Try again
        </Button>
      </div>
    );
  }

  const edges = [...links.data.outgoing, ...links.data.incoming].filter(isRelationshipEdge);

  return (
    <div className="flex flex-col gap-4">
      {edges.length === 0 ? (
        <EmptyState
          title="Nothing linked yet"
          description="Say who they answer to, where they turn up, and which systems they are built around. A cast is worth more connected."
        />
      ) : (
        <ul className="flex flex-col gap-2">
          {edges.map((edge) => (
            <EdgeRow
              key={edge.relationship.id}
              edge={edge}
              unlinking={unlink.isPending}
              onUnlink={(relationshipId) =>
                unlink.mutate({ entityId: character.id, relationshipId })
              }
            />
          ))}
        </ul>
      )}

      {unlink.isError && (
        <p className="text-xs text-error">
          {apiErrorMessage(unlink.error, 'Could not remove that link.')}
        </p>
      )}

      {character.status !== 'archived' && <LinkForm projectId={projectId} character={character} />}
    </div>
  );
}
